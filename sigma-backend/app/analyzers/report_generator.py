...
from app.analyzers.report_helpers import (
    BEIJING_TZ as BEIJING_TZ,
)
from app.analyzers.report_helpers import (
...
from app.analyzers.report_helpers import (
    _sentiment_for_item as _sentiment_for_item,
)
...
async def _resolve_report_llm_runtime(
    ...
    # Use user-level model if explicitly set; otherwise fall back to the
    # provider's default model so we never send e.g. a Claude model name to
    # the Qwen endpoint.  system_model is only used when the resolved provider
    # matches the system-level provider (prevents cross-provider mismatch).
    if user_model:
        model = str(user_model)
    elif system_model and provider == _normalize_provider(system_provider):
        model = str(system_model)
    else:
        model = DEFAULT_PROVIDER_MODELS.get(provider, str(settings.default_llm_model))
